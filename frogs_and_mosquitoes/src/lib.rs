...
pub fn eat_mosquitoes(frogs: Vec<(i32, i32)>, mosquitoes: Vec<(i32, i32)>) -> Vec<(i32, i32)> {
            ...
            if let Some((prev_x, (prev_t, _))) = prev_rane {
                if prev_x + prev_t >= *x {
                    ...
                    if prev_x + prev_t < *x + *t {
                        to_insert
                            .push((prev_x + prev_t + 1, (*t - (prev_x + prev_t - *x + 1), *i)));
                    }
                }
            }
        }
    ...
    for (pos, dim) in mosquitoes.iter() {
        ...
        if let Some((pred_x, (pred_t, pred_i))) = frog_bst.range(..=*pos).next_back() {
            if pred_x + pred_t >= *pos {
                ...
                while let Some((succ_x, (succ_t, succ_i))) =
                    frog_bst.range((curr_frog + 1)..).next()
                {
                    if pred_x + (updated_len) >= *succ_x {
                        ...
                        if pred_x + updated_len < succ_x + succ_t {
                            to_insert.push((
                                *pred_x + updated_len + 1,
                                (*succ_t - (*pred_x + updated_len - *succ_x + 1), *succ_i),
                            ));
                        }
                    } else {
                ...
                while let Some((old_mosq_pos, old_mosq_dim)) =
                    mosq_bst.range((mosq_index + 1)..).next()
                {
                    if pred_x + updated_len >= *old_mosq_pos {
                        ...
                        if let Some((update_frog_x, (update_frog_t, update_frog_i))) =
                            to_insert.pop()
                        {
                            to_insert.push((
                                update_frog_x,
                                (update_frog_t + *old_mosq_dim, update_frog_i),
                            ));
