...
impl SegTree {
    ...
    fn merge(
        ...
        let mut curr_seq: Vec<(i32, Option<usize>, Option<usize>)> =
        ...
        for i in left..=curr_seq_end {
            ...
            if j >= left_len {
                // the current element is bigger than all values (if any) in the child array
            ...
            } else if left_seq[j].0 == arr[i] {
                // values in parent and child perfectly match
                curr_elem.1 = Some(j); // so we link them
            } else {
                // not a perfect match, this means the value before was smaller than the curr_elem value
                if j == 0 {
                    curr_elem.1 = None;
                } else {
    ...
    fn smaller_values_rec(
        ...
        match overlap_type {
            ...
            Overlap::PARTIAL => {
                ...
                let left_res = if curr_node.seq[val_index.unwrap()].1 == None {
                    0
                } else {
                    self.smaller_values_rec(
                        query_interval,
                        curr_node.seq[val_index.unwrap()].1,
                        curr_node_index * 2 + 1,
                    )
                };
                let right_res = if curr_node.seq[val_index.unwrap()].2 == None {
                    0
                } else {
                    self.smaller_values_rec(
                        query_interval,
                        curr_node.seq[val_index.unwrap()].2,
                        curr_node_index * 2 + 2,
                    )
                };
    ...
    fn tuple_bin_search(
        arr: &Vec<(i32, Option<usize>, Option<usize>)>,
        len: usize,
        key: i32,
    ) -> Option<usize> {
        let mut low = 0;
